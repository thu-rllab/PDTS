    ...
    try:
        while True:
            ...
            if cmd == 'step':
                ...
                randomized_val = data
                env.randomize(randomized_val) # 怎么随机生成环境的？在wrappers.py中。
                remote.send(None)
...
class RandomizedSubprocVecEnv(VecEnv):
    ...
    def randomize(self, randomized_values):
        ...
        for remote, val in zip(self.remotes, randomized_values): # self.remotes有十个环境，randomized_values (10,1)
            remote.send(('randomize', val)) # remotes一个环境，randomized_values (1)
